...
```{julia}
MetaData(NASAPower.Source())
```

```{julia}
MetaData(OpenMeteoArchive.Source())
```
...
```{julia}
files = fetch(NASAPower.Source(), (-74.0, 40.7),
    Date(2024, 7, 1), Date(2024, 7, 14);
    variables = [:T2M, :PRECTOTCORR])
```
...
# NASA POWER
files_power = fetch(NASAPower.Source(), (-74.0, 40.7),
    Date(2024, 7, 1), Date(2024, 7, 14);
    variables = [:T2M])
...
# Open-Meteo Archive
files_meteo = fetch(OpenMeteoArchive.Source(), (-74.0, 40.7),
    Date(2024, 7, 1), Date(2024, 7, 14);
    variables = [:temperature_2m_mean], frequency = Frequency.daily)
